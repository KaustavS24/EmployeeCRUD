...
public class Dao {
	...
	public boolean updateEmployeeName(String name, long id)
	{
		try 
		{
			...
			PreparedStatement st = con.prepareStatement("update Employee set empName=? where empId=?");
			st.setString(1, name);
			st.setLong(2, id);
			int rs = st.executeUpdate();
			if(rs!=0)
	...
	public boolean updateEmployeeAge(int age, long id)
	{
		try 
		{
			...
			PreparedStatement st = con.prepareStatement("update Employee set empAge=? where empId=?");
			st.setInt(1, age);
			st.setLong(2, id);
			int rs = st.executeUpdate();
			if(rs!=0)
	...
	public boolean updateEmployeeAddress(String address, long id)
	{
		try 
		{
			...
			PreparedStatement st = con.prepareStatement("update Employee set empAddress=? where empId=?");
	...
	public long getLastEmployeeId()
	{
		...
		try 
		{
			...
			PreparedStatement st = con.prepareStatement("select empId from Employee where rownum <=1 order by empId desc");
			ResultSet rs = st.executeQuery();
			while(rs.next())
			{
				id = rs.getLong(1);
